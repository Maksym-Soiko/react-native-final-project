...
export function validateDto(dtoClass: any, source: "body" | "query" = "body") {
    return async (ctx: Context, next: Next) => {
        ...
        const dtoAsObject = dtoInstance as unknown as object;

        const errors = await validate(dtoAsObject, {
            whitelist: true,
        ...
        if (errors.length) {
            const messages = errors
                .map((e) => {
                    ...
                    if (!cons) {
                        return Object.values(e.children?.reduce((acc: any, c: any) => ({ ...acc, ...(c.constraints || {}) }), {}) || {}).join(", ");
                    }
